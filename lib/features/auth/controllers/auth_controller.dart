...
class AuthController extends GetxController implements GetxService {
  ...
  Future<void> registerRestaurant(
    ...
    List<XFile?> files = [];
    if (_pickedLogo != null) files.add(_pickedLogo);
    if (_pickedCover != null) files.add(_pickedCover);
    for (var doc in additionalDocuments) {
      if (doc != null && doc.files.isNotEmpty) {
        files.add(XFile(doc.files.first.path!));
      }
    }

    bool isFileSizeValid = await authServiceInterface.checkTotalFileSize(files);
    if (!isFileSizeValid) {
      _isLoading = false;
      update();
      return;
